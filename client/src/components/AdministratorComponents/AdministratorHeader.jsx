...
const AdministratorHeader = ({ onMenuToggle }) => {
    ...
    const fetchUserProfile = async () => {
      ...
      if (storedUser) {
        try {
          ...
          console.error("Error fetching administrator profile:", err);
        }
      }
    };
  ...
  const getProfileImageSrc = () => {
    ...
    return "/assets/superadminpic.png";
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:gap-6 w-full sm:w-auto sm:absolute sm:top-6 sm:right-8 justify-end sm:justify-start">
        ...
        {/* User avatar */}
        <Link to="/administrator/profilepage">
          <img
            src={getProfileImageSrc()}
            alt="User"
            className="w-6 h-6 sm:w-8 sm:h-8 rounded-full object-cover border-2 border-white shadow-sm cursor-pointer hover:scale-105 transition-transform flex-shrink-0"
          />
        </Link>
