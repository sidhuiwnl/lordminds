...
const SuperAdminUpload = () => {
  ...
  const fetchAllTopics = async () => {
    try {
      const res = await fetch(`${API_BASE}/topics/all-topics`);
  ...
  const fetchTopicWithSubTopic = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Failed to fetch topics with subtopics");
      const data = await res.json();
  ...
  const handleOverviewSubmit = async (e) => {
    ...
    try {
      ...
      const topic = allTopics.find((t) => String(t.topic_id) === String(formData.topicId));
  ...
  // Derive overview rows for table display
  ...
  const assignmentTotalPages = Math.max(1, Math.ceil(assignmentData.length / rowsPerPage));
  const overviewTotalPages = Math.max(1, Math.ceil(overviewRows.length / rowsPerPage));
  ...
  const getSubTopicsForTopic = (topicId) => {
    const topic = topicWithSub.find((t) => String(t.topic_id) === String(topicId));
  ...
  const topicOptions = mapOptions(allTopics, "topic_id", "topic_name");
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="flex items-end">
                <button
                  ...
                  className="h-[38px] px-4 bg-blue-500 text-white rounded-lg hover:bg-blue-600"
                >
                  + Add Topic
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setField("subTopicName", e.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setField("noOfSubTopic", e.target.value)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setField("overviewVideo", e.target.value)}
      ...
      {selectedTab === "upload-questions" && (
        <>
          <form onSubmit={handleMcqSubmit} className="bg-white rounded-lg shadow p-4 lg:p-6 mb-6">
            <h2 className="text-lg lg:text-xl font-semibold text-gray-800 mb-4 lg:mb-6">
              Fill the details to Upload Questions
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 lg:gap-6 mb-6">
              <div>
                ...
                <Select
                  options={topicOptions}
                  value={topicOptions.find((o) => String(o.value) === String(formData.mcqTopicId)) || null}
                  onChange={(opt) => setField("mcqTopicId", opt?.value || "")}
                  placeholder="Select Topic"
                  isClearable
                />
              </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
          <div className="bg-white rounded-xl p-6 shadow-lg w-80">
            ...
            <input
              type="text"
              value={topicNameModal}
              onChange={(e) => setTopicNameModal(e.target.value)}
