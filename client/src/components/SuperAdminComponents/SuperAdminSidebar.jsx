...
const SuperAdminSidebar = ({ isOpen, onClose, isMobile = false }) => {
  ...
  const activeMenu = (() => {
    if (location.pathname.includes("/superadmin/superadminhome")) return "home";
    if (location.pathname.includes("/superadmin/reports")) return "reports";
    if (location.pathname.includes("/superadmin/uploads")) return "upload";
    if (location.pathname.includes("/superadmin/access-creation")) return "access-creation";
    return "";
  })();
  ...
  const sidebarClasses = `
    ...
    ${
      isMobile
        ? `fixed inset-y-0 left-0 z-50 w-72 sm:w-80 -translate-x-full ${
  ...
  return (
    <div className={sidebarClasses}>
      ...
      <nav className="flex-1 p-1 sm:p-2 lg:pl-4 lg:py-6 space-y-1">
        {/* Home */}
        <button
          onClick={() => handleMenuClick("/superadmin/superadminhome")}
        ...
        {/* Reports */}
        <button
          onClick={() => handleMenuClick("/superadmin/reports")}
          className={`flex items-center gap-2 sm:gap-3 w-full px-2 sm:px-3 py-2 sm:py-3 font-medium transition-all duration-200 rounded-l-3xl text-xs sm:text-sm lg:text-base ${
            activeMenu === "reports"
              ? "bg-white text-[#1b65a6] shadow-lg transform scale-[1.02]"
              : "bg-transparent hover:bg-blue-500/30 hover:shadow-md"
        ...
        {/* Upload */}
        <button
          onClick={() => handleMenuClick("/superadmin/uploads")}
          className={`flex items-center gap-2 sm:gap-3 w-full px-2 sm:px-3 py-2 sm:py-3 font-medium transition-all duration-200 rounded-l-3xl text-xs sm:text-sm lg:text-base ${
            activeMenu === "upload"
              ? "bg-white text-[#1b65a6] shadow-lg transform scale-[1.02]"
              : "bg-transparent hover:bg-blue-500/30 hover:shadow-md"
        ...
        {/* Access Creation */}
        <button
          onClick={() => handleMenuClick("/superadmin/access-creation")}
          className={`flex items-center gap-2 sm:gap-3 w-full px-2 sm:px-3 py-2 sm:py-3 font-medium transition-all duration-200 rounded-l-3xl text-xs sm:text-sm lg:text-base ${
