...
const SuperAdminAccessCreation = () => {
  ...
  const handleSelectChange = (selectedOptions) => {
    setFormData(prev => ({ 
      ...prev, 
      selectedDepartments: selectedOptions ? selectedOptions.map(option => option.value) : [] 
    }));
  };
  ...
  const handleSubmit = (e) => {
    ...
    console.log({ selectedAccessType, ...formData });
  };
      ...
      return (
        <>
          ...
          <div className="flex flex-col lg:flex-row items-start gap-4">
            ...
            <Select
              isMulti
              options={onboardingDepartments.map(dept => ({ value: dept, label: dept }))}
              value={formData.selectedDepartments.map(dept => ({ value: dept, label: dept }))}
