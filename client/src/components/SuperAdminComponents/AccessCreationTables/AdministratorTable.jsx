...
const EditAdminModal = ({ admin, onClose, onUpdateSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (username !== admin.username) formData.append("username", username);
    ...
    >
      ...
      >
        <h3 className="text-xl font-semibold text-gray-900 mb-4">
          Edit Administrator: {admin.username}
        </h3>
...
/* ---------------- Admin Table ---------------- */
export const AdminTable = ({ admins, rowsPerPage = 10, onPageChange, refreshData }) => {
  ...
  useEffect(() => {
    if (onPageChange) onPageChange("admin", adminPage);
  }, [adminPage]);
  ...
  const adminTotal = filteredAdmins.length;
    ...
    <div className="bg-white rounded-lg shadow overflow-hidden mt-6">
      ...
      <div className="p-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center">
        <p className="text-sm text-gray-700">
          Showing {(adminPage - 1) * rowsPerPage + 1} to{" "}
          {Math.min(adminPage * rowsPerPage, adminTotal)} of {adminTotal} entries
        </p>
